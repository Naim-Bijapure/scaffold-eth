...
function Home({ baseURL, localChainId, userSigner, address, readContracts, writeContracts, tx }) {
  ...
  const onMint = async () => {
    try {
      ...
      console.log("n-request: ", request);
      // // @ts-ignore
      const relayResponse = await GelatoRelaySDK.relayWithSyncFee(request);
      console.log("n-relayResponse: ", relayResponse);
  ...
  const checkTaskStatus = async taskId => {
    try {
      ...
      console.log("n-statusResponse: ", statusResponse);
      ...
      if (statusResponse["taskState"] === "CheckPending") {
        await Sleep(1000);
